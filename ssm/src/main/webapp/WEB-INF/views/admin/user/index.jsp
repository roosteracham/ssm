				...
				<div class="modal-dialog" role="document">
					<div class="modal-content">
						...
						<div class="modal-body">
							<div class="container-fluid">密码将被重置为123123，确定重置密码？</div>
						</div>
						...
						<div class="modal-body">
							<div class="form-horizontal">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input id="emailAddr" type="text" class="form-control"
											placeholder="请输入邮箱地址">
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<button id="setDefaultPwd" class="btn btn-success"
												onclick="document.getElementById('auth').value=123123">
											初始密码为123123
										</button>
									</div>
								</div>
...
<script>
	$(document).ready(function() {
		...
		$("button.btn-primary.btn-xs").click(function(){
			...
			var zNodes;
			$.ajax({
				url : ctx + '/admin/role/roleZtree?userId='+userid,
				type : 'get',
				async : false,
				success : function(result) {
					result = $.parseJSON(result);
		...
		$("#btnSubmit").click(function() {
			...
			var emailAddr=$("#emailAddr").val().trim();
			var password=$("#auth").val().trim();
			if (emailAddr.length == 0) {
				$("#emailAddr").parent().addClass("has-error");
				$("#emailAddr").focus();
				setTimeout(function() {
					$("#emailAddr").parent().removeClass("has-error");
				}, 1500);
				return;
			}
			...
			if(isShowPassword){
				if (password.length == 0) {
					$("#auth").parent().addClass("has-error");
					$("#auth").focus();
					setTimeout(function() {
						$("#auth").parent().removeClass("has-error");
					}, 1500);
					return;
				}
			}
			...
			console.info("nickname:"+emailAddr);
			console.info("roleIds:"+roleIds);
			addOrUpdateUser(id, username, password, emailAddr, roleIds)
			
		});
		
		function checkUsername(name) {
			$.ajax({
				url : ctx + '/c1/checkUsername',
				type : 'get',
				async : false,
				data : {
					'name' : name
